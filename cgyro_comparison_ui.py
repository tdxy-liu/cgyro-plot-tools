...
class CgyroUiMixin:
    ...
    def _init_options(self):
        ...
        self.flux_xaxis_combo = ttk.Combobox(
            ...
            values=["v.s ky", "v.s kx (estimated)", "v.s Time", "v.s 2D"],
            state="readonly",
            width=18
        )
        ...
        self.zf_xaxis_combo = ttk.Combobox(
            ...
            values=["vs Time", "vs kx", "vs Gamma_Linear"],
            state="readonly",
            width=15
        )
        ...
        self.zf_formula_canvas.get_tk_widget().pack(fill=tk.BOTH, expand=True)
    ...
    def _render_zf_formula_math(self, mode_text):
        ...
        elif mode == "vs gamma_linear":
            lines = [
                r"Zonal ExB vs $\gamma_{lin}(k_y)$",
                r"$\phi_{ZF}\!\rightarrow\!\Omega(k_x,t)=-k_x^2\phi_{ZF}(k_x,t)$",
                r"$\omega_{ZF}(t)=\left(\sum_{k_x}|\Omega(k_x,t)|^2\right)^{1/2}$",
                r"$\langle \omega_{ZF}\rangle_t$ is compared with $\gamma_{lin}(k_y)$ from file.",
            ]
        else:
            lines = [
    ...
    def _build_effective_plot_type(self):
        ...
        if plot_type_selection == "Flux":
            ...
            elif zf_xaxis == "vs gamma_linear":
                plot_type = "ZF ExB Shearing vs Gamma Linear"
                display_plot_type = "Zonal ExB Shearing Rate (vs Gamma_Linear)"
            else:
