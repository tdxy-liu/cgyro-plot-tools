    ...
    def _render_energy_balance_formula_math(self):
        ...
        if mode_key == "zf":
            ...
            lines = [
                ...
                r"$\mathrm{Quantity} \in \{T,\ N,\ T\!-\!N,\ D_r,\ D_\theta,\ D_c,\ D_Z,\ entropy\}$",
                r"$\mathrm{X\!-\!axis} \in \{t,\ k_y,\ k_x,\ (k_x,k_y)\}$",
                r"$D_Z(k_x,k_y)=\left\langle D_r+D_\theta+D_c\right\rangle_t$",
                r"$\mathrm{DZ\ map:\ real\ triad\ channels}\ 6+7+8,\ \mathrm{selected\ species;\ signed\ values.}$",
