...
class CgyroPlottingMixin:
    ...
    def _apply_axis_labels(self, plot_type):
        ...
        if plot_type == "Frequency":
            ...
            x_label = r"$k_x \rho_s$"
            y_label = r"$\omega_{E\times B}^{ZF}\ (c_s/a)$"
        elif plot_type == "ZF ExB Shearing vs Gamma Linear":
            x_label = r"$k_y \rho_s$"
            y_label = r"$\gamma_{lin},\ \langle \omega_{E\times B}^{ZF} \rangle\ (c_s/a)$"
            ...
            try:
                ...
                if np.isfinite(tau) and tau > 0.0:
                    self.ax.plot(
                        ...
                        linestyle='-.',
                        linewidth=1.0,
                        label=f"{label} fit (l_corr={tau:.3g})",
                    )
    ...
    def _plot_flux(self, data, label, plot_type, t_indices, t_start, t_end, species_override_index):
        """Plot flux diagnostics versus ky, kx (estimated), or time."""
        ...
        if "vs ky" in plot_type:
            ...
            if is_decomp:
                ...
                if len(t_valid) > 0:
                    ...
                    label_total = f"{label} (Total) (Avg: {t_start_plot:.1f}-{t_end_plot:.1f})"
                else:
                    y_total = flux_ky_t[:, -1]
            ...
            if len(t_valid) > 0:
                ...
                label = f"{label} (Time Avg: {t_start_plot:.1f}-{t_end_plot:.1f})"
            else:
                y = flux_ky_t[:, -1]
        ...
        if "vs Time" in plot_type:
            ...
            if is_decomp:
                ...
                label_total = f"{label} (Total) {mean_str}"
                ...
                for f_idx in range(n_fields):
                    ...
                    fname = field_names[f_idx] if f_idx < len(field_names) else f"Field{f_idx}"
                    label_field = f"{label} ({fname}) {mean_str_f}"
    ...
    def _plot_flux_vs_kx_estimated(
        ...
        """Estimate and plot flux-vs-kx using spectral ES/EM proxy channels."""
        try:
        ...
        if t_valid.size > 0:
            ...
            label_total = f"{label} (Estimated ES+EM) (Avg: {t_start:.1f}-{t_end:.1f})"
        else:
        ...
        if is_decomp:
            ...
            if y_phi is not None:
                self.ax.plot(x[:len(y_phi)], y_phi[:len(x)], linestyle='--', marker='x', label=f"{label} (ES: " + r"$\phi$" + ")")
            if y_apar is not None:
                self.ax.plot(x[:len(y_apar)], y_apar[:len(x)], linestyle='--', marker='x', label=f"{label} (EM: " + r"$A_\parallel$" + ")")
            if y_bpar is not None:
                self.ax.plot(x[:len(y_bpar)], y_bpar[:len(x)], linestyle='--', marker='x', label=f"{label} (EM: " + r"$B_\parallel$" + ")")
    ...
    def _plot_zf_exb_shearing_vs_gamma_linear(self, data, label, t_indices, t_start, t_end):
        """Compare nonlinear averaged zonal shearing against linear gamma(ky)."""
        # 1) Nonlinear macro ZF shearing scalar: time-window average of shearing-rate time trace.
        t, shear_t = self._compute_zf_exb_shearing_time_series(data, label)
        if t is None:
            return

        n_t = min(t.size, shear_t.size)
        if n_t <= 0:
            print(f"No time samples for nonlinear ZF shearing in {label}")
            return
        ...
        valid_t = valid_t[(valid_t >= 0) & (valid_t < n_t)]
        if valid_t.size > 0:
            shear_subset = shear_t[valid_t]
            omega_zf_mean = float(np.mean(shear_subset))
            omega_zf_std = float(np.std(shear_subset))
            t_info = f"{t_start:.1f}-{t_end:.1f}"
        else:
            omega_zf_mean = float(np.mean(shear_t))
            omega_zf_std = float(np.std(shear_t))
            t_info = "full"

        # 2) Linear gamma(ky) curve from exported file.
        file_path = self._resolve_linear_gamma_file_path(data)
        if file_path is None:
            print(
                f"Linear spectrum file not found for {label}. "
        ...
        finite_mask = np.isfinite(ky_lin) & np.isfinite(gamma_lin)
        if not np.any(finite_mask):
            print(f"No finite ky/gamma points in {file_path} for {label}")
            return

        x = np.asarray(ky_lin[finite_mask], dtype=float).reshape(-1)
        y = np.asarray(gamma_lin[finite_mask], dtype=float).reshape(-1)
        order = np.argsort(x)
        x = x[order]
        y = y[order]
        if x.size == 0:
            print(f"No usable ky points in {file_path} for {label}")
            return
        ...
        # <omega_ZF> horizontal line
        self.ax.plot(
            [x[0], x[-1]],
            [omega_zf_mean, omega_zf_mean],
            linestyle='--',
            color=line.get_color(),
            linewidth=1.8,
            label=(
                f"{label} "
                + r"$\langle\omega_{E\times B}^{ZF}\rangle$"
                + f"={omega_zf_mean:.2e} (std={omega_zf_std:.2e}, t={t_info})"
            ),
        )
        self.ax.set_xlabel(r'$k_y \rho_s$')
        self.ax.set_ylabel(r'$\gamma_{lin},\ \langle \omega_{E\times B}^{ZF} \rangle\ (c_s/a)$')

    def _plot_zf_exb_shearing_spectrum(self, data, label, t_indices, t_start, t_end):
        """Plot zonal ExB shearing spectrum versus kx."""
        kx, phi_kx_t, t = self._get_zf_exb_phi_kx_t(data, label)
        if kx is None:
            return
    ...
    def _dispatch_single_case_plot(
        ...
        """Route one case to exact plotting backend according to `plot_type`."""
        exact_handlers = {
            "ZF ExB Shearing Rate": lambda: self._plot_zf_exb_shearing_rate(data, label, t_indices, t_start, t_end),
            "ZF ExB Shearing Spectrum": lambda: self._plot_zf_exb_shearing_spectrum(data, label, t_indices, t_start, t_end),
            "ZF ExB Shearing vs Gamma Linear": lambda: self._plot_zf_exb_shearing_vs_gamma_linear(data, label, t_indices, t_start, t_end),
            "Fluctuation 2D": lambda: self._plot_fluctuation_2d(data, label, plot_type, t_indices, t_start, t_end),
            "Fluctuation 2D vs xt": lambda: self._plot_xt_fluctuation_contours(data, label, plot_type, t_indices, t_start, t_end),
            "Integration Error": lambda: self._plot_other_error(data, label),
            "Radial Correlation (rcorr_phi)": lambda: self._plot_other_rcorr_phi(data, label, t_indices, t_start, t_end),
            "POD Parity": lambda: self._plot_other_apar_pod_parity(data, label, t_indices, t_start, t_end),
        }
    ...
    def _plot_single_case(self, data, label, plot_type, species_override_index=None):
        """Dispatch one case to the corresponding specialized plotting routine."""
